...
struct SettingsSearchEngineTests {
	...
	private let connected = SettingsSearchEngine.Availability(
		isConnected: true, isDIYHardware: false, currentValues: [])
...
struct DocumentationSearchTests {
	...
	func ranking() throws {
		...
		guard !results.isEmpty else { return }  // no MQTT page in this bundle
		#expect(
	...
	@Test("Results are stable across calls")
	func deterministic() throws {
		#expect(DocumentationSearch.search("radio").map(\.id)
				== DocumentationSearch.search("radio").map(\.id))
	}
}
