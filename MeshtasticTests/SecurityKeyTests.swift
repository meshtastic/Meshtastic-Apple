...
struct SecurityKeyTests {
	...
	// MARK: - Packet authenticity

	// Choosing Strict deliberately does not commit: it raises a pending flag that the
	// confirmation alert reads. A binding that rebuilds this state from the stored
	// message on every read throws that flag away as soon as it is set, which is how
	// Strict became unreachable on a device while the suites stayed green.
	@Test("Strict waits for a confirmation instead of committing")
	func strictNeedsConfirming() {
		var selection = PacketAuthenticitySelectionState(selected: .compatible)

		selection.propose(.strict)
		#expect(selection.pendingStrict, "the confirmation has to be asked for")
		#expect(selection.selected == .compatible, "and nothing changes until it is answered")

		selection.confirmStrict()
		#expect(selection.selected == .strict)
		#expect(!selection.pendingStrict)
	}

	@Test("Cancelling a Strict confirmation leaves the policy alone")
	func cancellingStrictKeepsThePolicy() {
		var selection = PacketAuthenticitySelectionState(selected: .compatible)
		selection.propose(.strict)
		selection.cancelStrict()
		#expect(selection.selected == .compatible)
		#expect(!selection.pendingStrict)
	}

	@Test("Every other level commits straight away")
	func otherLevelsCommitImmediately() {
		var selection = PacketAuthenticitySelectionState(selected: .strict)
		selection.propose(.compatible)
		#expect(selection.selected == .compatible, "leaving Strict needs no confirmation")
		#expect(!selection.pendingStrict)
	}

	// The transient half of the state is the part a derived binding loses, so pin that
	// rebuilding from a stored policy cannot carry a pending confirmation.
	@Test("State rebuilt from a stored policy has nothing pending")
	func rebuiltStateHasNoPendingConfirmation() {
		var selection = PacketAuthenticitySelectionState(selected: .compatible)
		selection.propose(.strict)
		#expect(selection.pendingStrict)

		let rebuilt = PacketAuthenticitySelectionState(selected: selection.selected)
		#expect(!rebuilt.pendingStrict, "which is why the screen holds this as real state")
	}
}
