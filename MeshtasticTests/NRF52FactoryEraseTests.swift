...
struct NRF52FactoryEraseTests {
	...
	@Test func seededImagesMatchTheAuditedFixture() throws {
		let s140611 = try #require(NRF52FactoryErase.eraseEntry(for: .s140_6_1_1))
		#expect(s140611.image.fileName == "nrf_erase2.uf2")
		#expect(s140611.image.sha256 == "4b778a3def19854415db64cb51bfd29c15b11cc46006353dd518f62d09efe3fe")
		#expect(s140611.expectedFirstTargetAddress == 0x26000)
		#expect(s140611.expectedFamilyID == nil, "the SoftDevice path is gated on the address, not a family")

		let s140730 = try #require(NRF52FactoryErase.eraseEntry(for: .s140_7_3_0))
		#expect(s140730.image.fileName == "nrf_erase_sd7_3.uf2")
		#expect(s140730.image.sha256 == "13941bedce009e61255c37b1524d11ca604e88c38e7588bb8b391e2998da468f")
		#expect(s140730.expectedFirstTargetAddress == 0x27000)

		// The images are served by the api resource now, not a commit-pinned web-flasher path.
		#expect(s140611.image.url.absoluteString == "https://api.meshtastic.org/resource/maintenanceUf2/asset/nrf_erase2.uf2")
	}
	...
	@Test func seededBootloaderImageMatchesTheContract() throws {
		let entry = try #require(NRF52FactoryErase.bootloaderEraseEntry(forVolumeFamily: 0x4D45_5348))
		#expect(entry.image.fileName == "meshtastic_factory_erase.uf2")
		#expect(entry.image.sha256 == "6ef3146505c40079ee9e7e692448e40a793dad636f55d1545063299d28908f0d")
		#expect(entry.image.url.lastPathComponent == entry.image.fileName)
		#expect(entry.image.url.absoluteString == "https://api.meshtastic.org/resource/maintenanceUf2/asset/meshtastic_factory_erase.uf2")
		#expect(entry.expectedFamilyID == 0x4D45_5348)
		#expect(entry.expectedFirstTargetAddress == nil, "its target address is 0 by design, so no address check applies")
	}

	/// The decision that keeps this app and Android in step: a drive advertising a family the
	/// manifest does not carry — or advertising none at all — resolves to nil here, and the
	/// caller falls through to the SoftDevice path. It does not refuse. That path is separately
	/// gated on a SoftDevice match, the digest, and the start-address cross-check.
	@Test func anUnmatchedOrAbsentFamilyFallsThroughRatherThanRefusing() {
		#expect(NRF52FactoryErase.bootloaderEraseEntry(forVolumeFamily: nil) == nil)
		#expect(NRF52FactoryErase.bootloaderEraseEntry(forVolumeFamily: 0xADA5_2840) == nil)
		#expect(NRF52FactoryErase.bootloaderEraseEntry(forVolumeFamily: 0) == nil)
	}
