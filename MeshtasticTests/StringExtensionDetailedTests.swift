...
struct IntervalConfigurationDetailedTests {
	...
	@Test func allCases_count() {
		...
		// would still pass if a case were swapped for an unrelated one, so the newest are
		// named outright.
		#expect(IntervalConfiguration.allCases.count == 15)
		...
		#expect(IntervalConfiguration.allCases.contains(.waitBluetooth))
	}
