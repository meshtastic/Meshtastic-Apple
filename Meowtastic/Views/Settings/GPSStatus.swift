...
struct GPSStatus: View {
	...
	@ObservedObject
	var locationsHandler: LocationsHandler = LocationsHandler.shared
	...
	var body: some View {
		if let newLocation = locationsHandler.locationsArray.last {
			...
			Label(
				"Coordinate \(String(format: "%.5f", newLocation.coordinate.latitude)), \(String(format: "%.5f", newLocation.coordinate.longitude))",
				systemImage: "mappin"
			)
			.font(largeFont)
			...
			HStack {
				Label(
					"Speed \(speed.formatted(.measurement(width: .abbreviated, numberFormatStyle: .number.precision(.fractionLength(0)))))",
					systemImage: "speedometer"
				)
				.font(largeFont)
				
				Label(
					"Accuracy \(speedAccuracy.formatted(.measurement(width: .abbreviated, numberFormatStyle: .number.precision(.fractionLength(0)))))",
