					...
					if bleManager.isSwitchedOn {
								...
								connectedPeripheral.peripheral.state == .connected
							{
								HStack {
									...
									VStack(alignment: .leading) {
										...
										Text("ble.name")
											.font(.callout)+Text(": \(bleManager.connectedPeripheral?.peripheral.name ?? "unknown".localized)")
											.font(.callout)
											.foregroundColor(Color.gray)

										if node != nil {
											Text("firmware.version")
												.font(.callout)+Text(": \(node?.metadata?.firmwareVersion ?? "unknown".localized)")
												.font(.callout)
												.foregroundColor(Color.gray)
										}
						...
						if !self.bleManager.isConnected {
							Section(header: Text("available.radios").font(.title)) {
								ForEach(
									bleManager.peripherals.filter({
										$0.peripheral.state == CBPeripheralState.disconnected
									})
									...
									HStack {
										...
										Button(action: {
											if
												UserDefaults.preferredPeripheralId.count > 0 && peripheral.peripheral.identifier.uuidString != UserDefaults.preferredPeripheralId
											{
												...
												do {
													try Persistence.shared.copyPersistentStores(
														to: url.appendingPathComponent("backup")
