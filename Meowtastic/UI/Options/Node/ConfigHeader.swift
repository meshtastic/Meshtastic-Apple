...
struct ConfigHeader<T>: View {
	...
	var body: some View {
		if let node {
			if node.metadata == nil && node.num != bleManager.connectedPeripheral?.num ?? 0 {
				Text("There has been no response to a request for device metadata over the admin channel for this node.")
					.font(.callout)
					.foregroundColor(.orange)
			}
			else if node.num != bleManager.connectedPeripheral?.num ?? 0 {
			...
			else if node.num == bleManager.connectedPeripheral?.num ?? -1 {
