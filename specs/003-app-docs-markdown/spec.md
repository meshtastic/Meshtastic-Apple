...
- **FR-029**: The AI assistant MUST be branded as **Chirpy** in all user-facing text and navigation titles. The assistant sheet MUST include Chirpy's avatar image (`AppIcon_Chirpy_Thumb`), a "Hi, I'm Chirpy!" headline, and adopt `.scrollDismissesKeyboard(.interactively)` so that scrolling the form dismisses the keyboard. The system prompt MUST establish Chirpy's persona as a friendly, concise expert on Meshtastic.
