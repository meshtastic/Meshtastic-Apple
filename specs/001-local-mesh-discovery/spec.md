...
- **FR-037** [PROPOSED]: The system MUST provide a MeshBeacon module configuration editor that reads the connected radio's `ModuleConfig.MeshBeaconConfig` and writes changes back via an `AdminMessage` (`setModuleConfig.meshBeacon`). [NEEDS CLARIFICATION: navigation home — Settings > Module Configuration alongside other modules, and/or a `meshtastic:///` deep link.]
...
- **FR-039** [PROPOSED]: The editor MUST let the user edit the beacon `broadcast_message` and MUST prevent saving a message longer than the firmware-enforced 100 bytes. [NEEDS CLARIFICATION: block-with-error vs. live truncation.]
...
- **FR-041** [PROPOSED]: The editor MUST let the user set `broadcast_interval_secs` and MUST enforce the firmware minimum of 3600 s (default 3600). [NEEDS CLARIFICATION: clamp silently vs. reject-with-message; and which interval choices to surface.]
- **FR-042** [PROPOSED]: The editor MUST let the user set the radio settings a beacon is transmitted on via the single-target scalar fields `broadcast_on_channel`, `broadcast_on_region`, and `broadcast_on_preset`. [NEEDS CLARIFICATION: does v1 also read/edit/preserve the repeated `broadcast_targets` multi-target list and `broadcast_send_as_node`, or only the single-target path? And whether to expose `FLAG_LEGACY_SPLIT` (split a combined beacon into separate `MESH_BEACON_APP` + `TEXT_MESSAGE_APP` packets) to users or manage it automatically.]
- **FR-043** [PROPOSED]: [NEEDS CLARIFICATION] Define whether beacons heard passively (listening enabled, no active scan) are captured and surfaced — e.g. delivered to the message inbox as the firmware comment implies, and/or shown in a dedicated Beacons list — and their persistence/retention model when there is no `DiscoverySession` to own them. The current build only logs such packets (see FR-029, which stores beacons only during a dwell).
