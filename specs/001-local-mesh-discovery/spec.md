...
- **FR-029**: The scan setup MUST present the selectable modem presets as a compact, adaptive grid of chips (multi-column, wrapping to the available width) rather than one full-width row per preset, so the picker stays short on iPhone and reads as multiple columns on iPad/Mac. Each chip MUST show its selection state (a filled/accent-tinted style with a checkmark when selected, an unobtrusive outline when not), a beacon indicator when a beacon has advertised that preset, and a "popular" indicator when the preset is one of the most-common public-mesh presets (FR-030). Tapping a chip toggles its selection.
