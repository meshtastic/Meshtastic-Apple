...
**Decision**: Constitution Principle VIII requires fetching the live [Meshtastic Client Design Standards](https://raw.githubusercontent.com/meshtastic/design/refs/heads/master/standards/meshtastic_design_standards_latest.md) **at UI-implementation time**, not cached into this plan — the constitution's own wording gates "before making UI changes," i.e. the coding phase, and explicitly forbids relying on a summary. This document could not be reliably fetched during planning (tooling returned empty content despite a `200 OK`); implementation tasks for the estimate form, map control, and node-detail action MUST each re-fetch and check against the live document before finalizing that screen's layout, spacing, and iconography — this is a hard requirement carried into `tasks.md`, not an assumption made here.
