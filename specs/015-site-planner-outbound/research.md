...
**Status after T012's spike: inconclusive on this machine — not resolved false, not resolved true.** Recording exactly what was and wasn't established, rather than picking one to avoid leaving this ambiguous.

**What's confirmed:**
- **iOS Simulator: fully verified, real end-to-end success.** `CoverageEstimateBridgeLiveSpikeTests` (a genuine integration test against the live site.meshtastic.org, not a mock) passed in 6.3 seconds — real navigation, real JS execution inside the 1×1/alpha-0 `WKWebView` attached to the key window, a real simulation, and a real `onCoverage` callback received and parsed as a valid `FeatureCollection`. The core mechanism (`WKUserScript` shim → `WKScriptMessageHandler` → `MapDataManager.importFromData`) works exactly as designed on iOS.
- **Mac Catalyst: compiles clean.** A full app build for `platform=macOS,variant=Mac Catalyst` succeeds with no errors — `WKWebView`/`WKUserScript`/`WKScriptMessageHandler`/`UIWindow` are all available and type-check fine under Catalyst.

**What's still unknown, and why:** the same spike could not be run to completion under Mac Catalyst on this development machine, for two *environmental* reasons unrelated to the WebKit question itself:
1. The `MeshtasticTests` target's Mac Catalyst configuration resolves to a macOS 26.5 deployment target, but this machine runs macOS 26.4.1 — `xcodebuild test` refuses to run at all ("doesn't match... deployment target"). Overriding `MACOSX_DEPLOYMENT_TARGET` on the command line to work around it breaks a *different*, unrelated dependency (`swift-snapshot-testing`'s `AttachableAsImage` conformance requires the newer SDK) — so the test target is simply not runnable on this host right now, independent of anything in this feature.
2. Falling back to a manual spike (a temporary, DEBUG-only, launch-argument-triggered hook added to `ContentView.task`, mirroring `MarketingCapture`'s established pattern, then removed once done) hit a *different* wall: the Catalyst app, launched via `open -n MyApp.app --args ...` outside Xcode's own run/debug session, never presented a window at all (`System Events` reported zero windows for the running process, though the process itself stayed alive) — so the spike code never demonstrably ran. This reads like an ad-hoc-launch/signing-context issue specific to bypassing Xcode's debugger, not a WebKit-specific failure — but that's an inference, not a confirmed diagnosis.

**Recommendation carried into implementation**: run the same `CoverageEstimateBridgeLiveSpikeTests` suite via Xcode's own Test navigator (⌘U) targeting a Mac Catalyst run destination, on a machine whose installed macOS matches the test target's deployment target (or after that target's deployment target is corrected to something the SDK and the dev machine both satisfy) — that removes both confounds at once and gives a real answer. Until then, `CoverageEstimateBridge`'s design already commits to the safer choice (attached-but-invisible, not fully detached) as the working assumption, per the original rationale below.

**Original rationale for the attached-but-invisible design** (unchanged, still the implementation): an off-screen/zero-frame `WKWebView`'s JS execution and `WKUserContentController` message delivery under Catalyst's AppKit-hosted `NSView` bridging has known edge cases elsewhere in WebKit (some Catalyst builds have historically needed the WebView attached to a visible view hierarchy, even at 1×1 size, for its render/JS pump to run reliably in the background). This project already conditionally special-cases Mac Catalyst elsewhere (`#if targetEnvironment(macCatalyst)` in `MarketingCapture.swift`, entitlements split, etc.), so attaching the hidden WebView to the key window rather than leaving it unattached is the safer default `CoverageEstimateBridge` already implements — the open question is only whether that's *sufficient*, not whether it's the right first attempt.
