...
- **Beacon offers a named channel with an empty or default PSK**: [NEEDS CLARIFICATION: treat as the default public key (joinable) or reject the offer as not joinable? See research.md open question 7.]
- **Switch to this channel partially applies then fails**: [NEEDS CLARIFICATION: if the join sets the primary channel but the subsequent LoRa/region/preset apply fails, roll back the primary-channel change or leave it? See research.md open question 8.]
