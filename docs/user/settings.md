...
When sharing channels, the share screen shows a QR code and link, and — on iPhones with NFC hardware (iOS 18 or later) — a **Write to NFC Tag** button. Hold a writable, empty NFC tag near the top of your iPhone to save the channel link to it; tapping that tag on another phone opens the same add-or-replace channels flow the QR code does.
...
| Action | What it does |
|--------|--------------|
| **Write Contact to NFC Tag** | Saves your connected node's contact link to a writable, empty NFC tag. Hold the tag near the top of your iPhone. |
