...
| Bandwidth | Available under **Advanced** when **Use Preset** is off. In sub-GHz regions, a stored default value of 0 represents 250 kHz. In the 2.4 GHz region, custom mode requires an explicit bandwidth selection and disables Save until you choose a supported value. Choices are filtered for the selected region and connected radio. If a stored bandwidth is unsupported, the app shows a warning and prevents saving until you select a supported value. |
