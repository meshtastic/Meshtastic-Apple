...
| Bandwidth | Available under **Advanced** when **Use Preset** is off. Custom mode requires a specific bandwidth; the preset-mode default value cannot be reused. Choices are filtered for the selected region and connected radio. If a stored bandwidth is unsupported, the app shows a warning and prevents saving until you select a supported value. |
