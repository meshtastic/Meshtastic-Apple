...
`CBCentralManager` is created with a restore identifier, so when iOS kills the app in the background while it is connected and later relaunches it, CoreBluetooth calls `centralManager(_:willRestoreState:)` before `centralManagerDidUpdateState(_:)`. `BLETransport.handleWillRestoreState(dict:central:)` takes the first restored peripheral and, if it is `.connecting`, re-issues the connect and waits for `didConnect` before handing the connection to `AccessoryManager`.

That restore owns four pieces of transport state: the `restoreInProgress` discovery gate, `activeConnection`, the pending `restoredConnectContinuation`, and a `discoveredPeripherals` entry. Two rules keep them from leaking:

- **`beginRestore(for:)` and `endRestore(clearingConnection:)` are the only writers of the gate.** `restoreInProgress` suppresses all three scan entry points (`didDiscover`, the `discoverDevices()` setup task, and the `.poweredOn` re-scan), so a restore that exits without clearing it leaves Available Radios permanently empty with no error shown and no recovery short of relaunching — toggling Bluetooth does not help, because the `.poweredOn` re-scan is itself gated on the flag. The restore task clears it from a `defer`, so no current or future early return can latch it. `endRestore` also drops the pending continuation and, unless the restored connection is genuinely live, `activeConnection` — a connection that never reached `BLEConnection.connect()` is never torn down by `connectionDidDisconnect`, and while it sits in `activeConnection` the busy guards in `connect(to:)` reject every later connect attempt. That clear is identity-scoped to the connection the restore itself installed, and never touches `connectingPeripheral` (which the restore path does not set): a restore tearing down late must not drop the state of a `connect(to:)` that started in the meantime, or the same hang reappears from the other direction. The `discoveredPeripherals` entry the restore added is deliberately kept, so the user can still reconnect by hand after a failed restore; the periodic cleanup task prunes it once the radio stops being seen.
- **The wait for `didConnect` is bounded.** CoreBluetooth connect requests never expire, so if the radio is powered off or out of range at relaunch, none of `didConnect`, `didFailToConnect`, or `didDisconnectPeripheral` ever arrives. `awaitRestoredConnect(timeout:connect:cancelConnect:)` gives that wait a deadline (`BLETransport.restoredConnectTimeout`) and withdraws the pending request with `cancelPeripheralConnection` when it expires. The same wait also ends if the restore task is cancelled (a checked continuation is not cancellation-aware on its own, so this goes through `withTaskCancellationHandler`) or if a second restore supersedes it, and every one of those give-up paths withdraws the pending connect request. Each wait carries a generation number so a watchdog that wakes up just as its own wait was resolved cannot take down a later restore's continuation.

`connectionDidDisconnect(fromPeripheral:)` is the single funnel for "this peripheral is gone" and resolves a pending restored connect for the same reason. The restore short-circuits in `handleDidConnect` and `handleDidFailToConnect` are scoped to the peripheral being restored (`restoreInProgress` plus an identifier match), the same way the normal connect path checks `connectingPeripheral`; unscoped, a leftover restore continuation would answer an unrelated connect's `didConnect` and hang it.
...
- **Scanning must stop before Step 1.** CoreBluetooth scanning concurrently with the `FROMNUM` bonding subscription races the pairing handshake: iOS tears the PIN sheet down almost immediately and the subscription fails with `CBATTErrorInsufficientEncryption` (a documented CoreBluetooth interaction, not something a retry recovers from). `AccessoryManager+Connect`'s Step 0 calls `stopDiscovery()` before every Step 1 attempt (first try and retries alike), and `appDidBecomeActive()` will not resume the scan while `isConnecting` is true — the pairing sheet is out-of-process UI that can blip `scenePhase` the same way the Bluetooth-power alert did (#2139/#2161), and resuming the scan on that blip would reintroduce the race mid-handshake.
