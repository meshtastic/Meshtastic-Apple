...
The retry loop is cancellation-correct: it calls `Task.checkCancellation()` before each attempt and lets `Task.sleep(for:)` throw during the backoff, so a cancelled send cannot put another write on the wire. It checks once more before escalating an exhausted final attempt, preventing an intentional cancellation from starting a reconnect.

Write-with-response continuations live in `WriteContinuationStore`, keyed by UUID. The CoreBluetooth callback removes the oldest entry, while task cancellation removes its specific UUID; all access is actor-isolated. Disconnect drains every remaining entry with an error. This FIFO/keyed design assumes writes complete in order and that cancelling one write does not overlap another pending callback. Writes without response are handed to CoreBluetooth and return immediately because CoreBluetooth provides no completion callback to cancel.
...
- **An in-flight `send` is cancellation-aware without closing the socket.** `send` stores its checked continuation in `pendingSendContinuation` and wraps the wait in `withTaskCancellationHandler`. The `NWConnection` completion and cancellation handler race to claim that single slot; the winner resumes it and the other no-ops. Cancellation resumes with `CancellationError` but deliberately leaves the `NWConnection` alive, because a cancelled configuration import still needs the same connection to send `commit_edit_settings`. The operating system may already have accepted the payload, so this is abandon-and-move-on behavior rather than a guarantee that no bytes reach the radio. The single continuation slot requires callers to serialize sends; actor isolation alone does not prevent a second call from entering while the first is suspended.
