...
def main() -> None:
    ...
    if dump(catalog) != raw:
        sys.exit("  Localizable.xcstrings is not in the formatting this script writes; "
                 "open it in Xcode and save once, then rerun")
    strings = catalog["strings"]

    # Translations available per English text, from the literal keys.
    by_english: dict[str, dict[str, dict]] = {}
    for key, entry in strings.items():
        if key.startswith(REGISTRY_PREFIX):
            continue
        translations = {lang: unit for lang, unit in units(entry).items() if lang != "en"}
        if translations:
            by_english.setdefault(english(key, entry), {}).update(translations)
    ...
    if args.dry_run:
        ...
        return
    CATALOG.write_bytes(dump(catalog))
