...
**Jul 2026** — [Architecture](architecture.md) — Protobufs resynced to upstream `master` (v2.7.26-97-g9d589c1): added the `MEDIUM_TURBO` 500 kHz modem preset (wired through `ModemPresets` and its bandwidth/SNR/label switches, gated to 2.8+ firmware via `requiresFirmware2_8`), the `LORA_OTA_APP` PortNum (logged in the inbound packet dispatch), the `MESHBEACON_CONFIG` admin module-config type, the `SPA06` telemetry sensor, `DeviceProfile.is_unmessagable`/`is_licensed` fields, the Heltec `RC32`/`RC52`/`RCC6` hardware models, and the `TRACKER_T1000_E_PRO` → `MESH_TRACKER_X1` rename — all additive, no entity-mapping or migration changes.
