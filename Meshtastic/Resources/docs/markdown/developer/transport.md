...
- **Stopping the scan empties the peripheral cache, so the connect path cannot rely on it.** `stopScanning()` clears `discoveredPeripherals`, and Step 0 stops discovery before Step 1 connects — so by the time `transport.connect(to:)` runs, that cache is empty by construction. `BLETransport.resolvePeripheral(for:)` therefore treats the cache as a fast path only and falls back to `CBCentralManager.retrievePeripherals(withIdentifiers:)`, the API for reconnecting to a peripheral the system already knows by identifier without scanning for it first. That lookup consults the system's peripheral registry and does not start a scan, so it does not reopen the race above. A cache-only lookup here fails every connect attempt with "Peripheral not found" and never reaches `centralManager.connect(_:)`, so the radio sits advertising and never sees a connection attempt at all.
