...
- **A cancelled step is a failed step.** Steps park on cancellable suspensions (`sendWantDatabase` waits on a continuation, `waitForWantDatabaseResponse` waits on `wantDatabaseGate`), and a teardown resumes those with `CancellationError`. The retry loop rethrows that cancellation instead of moving on, so it reaches the outer failure handler. That handler treats `.retryStep` the same as `.retryAll`: it restarts the whole pipeline from Step 0, abandoning any remaining per-step attempts, until `maxRetries` is exhausted and `AccessoryError.tooManyRetries` is thrown. On a `.retryStep` step with attempts still remaining, an error recorded through `cancelCurrentlyExecutingStep(withError:)` is substituted for the bare cancellation. Steps with no per-step retries rethrow the cancellation directly, and the recorded error reaches the UI through `lastConnectionError` instead.
- **Steps that wait on the radio carry a timeout.** Step 1 connect (5s, or 90s on a first-ever BLE bond), Step 3 `wantConfig` (30s), Step 5 `wantDatabase` (10s, with three in-place attempts), and Step 5a's node-DB wait (120s) are the timed steps. The remaining steps only do local work, so a new step that waits on the link has to bring its own timeout. Step 5a is capped because the gate it waits on is only opened by a `configComplete` for `NONCE_ONLY_DB`, and `closeConnection()` resets that gate to closed; without a cap, a teardown racing that step would suspend the pipeline permanently. The cap is a backstop for that race rather than a service level for the dump, so it is set well above what a healthy large node DB on a slow link needs: Step 5a is `.retryAll`, so an early timeout would restart the whole pipeline and end in `tooManyRetries` with autoconnect trying again.
...
`isRunning` reports whether a machine is live and is cleared on every exit path, including a throw out of a retry delay. It is the predicate behind the reentrancy guard above, so a leaked `true` refuses every later connect. `AccessoryManager.didReceive(_:)` also re-reads it after tearing a connection down, so an error handled while a new connect is already in flight does not pull the UI back to `.discovering`.
