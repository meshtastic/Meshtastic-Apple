...
#if DEBUG
extension AccessoryManager {
	func simulateConnectedDevice(num: Int64, shortName: String, longName: String) {
		let deviceId = "performance-seed-\(num)".toUUIDFormatHash()
		var device = Device(
			id: deviceId,
			name: longName,
			transportType: .ble,
			identifier: "performance-seed-\(num)",
		...
		device.firmwareVersion = "2.7.0"
		device.hardwareModel = "TBEAM"
		device.lastUpdate = Date()

		devices.removeAll { $0.id == deviceId }
		devices.insert(device, at: 0)
		activeConnection = (device: device, connection: SimulatedPerformanceSeedConnection())
		activeDeviceNum = num
...
actor SimulatedPerformanceSeedConnection: Connection {
