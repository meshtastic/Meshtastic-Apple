...
extension AccessoryManager {
	...
	func handleMyInfo(_ myNodeInfo: MyNodeInfo) {
		...
		Task { @MainActor in
			if let myInfo = myInfoPacket(myInfo: myNodeInfo, peripheralId: connectedDeviceId, context: context) {
				updateDevice(key: \.name, value: myInfo.bleName ?? "Unknown")
				updateDevice(key: \.longName, value: myInfo.bleName ?? "Unknown")
	...
	func handleModuleConfig(_ moduleConfigPacket: ModuleConfig) {
		...
		Task { @MainActor in
			...
			case .cannedMessage:
				try? await getCannedMessageModuleMessages(destNum: deviceNum, wantResponse: true)
			default:
