...
struct SitePlannerCoverageClient: Sendable {
	...
	static func usesBrowserSitePlanner(for sitePlannerURL: URL) -> Bool {
...
@MainActor
final class SitePlannerBrowserCoverageClient: NSObject {
	// The public Site Planner site has no hosted prediction API. Drive its local
	// browser engine with a share-link config and capture the GeoJSON export.
	private var continuation: CheckedContinuation<Data, Error>?
	private var timeoutTask: Task<Void, Never>?
	private var webView: WKWebView?

	func generateContours(
		from sitePlannerURL: URL,
		request payload: SitePlannerCoverageRequest,
		timeoutInterval: TimeInterval = 240
	) async throws -> Data {
		try await withCheckedThrowingContinuation { continuation in
			self.continuation = continuation
			self.loadSitePlanner(sitePlannerURL, payload: payload, timeoutInterval: timeoutInterval)
		}
	}

	private func loadSitePlanner(
		_ sitePlannerURL: URL,
		payload: SitePlannerCoverageRequest,
		timeoutInterval: TimeInterval
	) {
		let userContentController = WKUserContentController()
		userContentController.addUserScript(
			WKUserScript(
				source: Self.exportCaptureScript,
		...
		timeoutTask = Task { @MainActor [weak self] in
			...
			self?.finish(.failure(SitePlannerCoverageError.timeout))
		}

		let url = Self.sitePlannerURL(from: sitePlannerURL, payload: payload)
	...
	private static func sitePlannerURL(from sitePlannerURL: URL, payload: SitePlannerCoverageRequest) -> URL {
		var components = URLComponents()
		components.scheme = sitePlannerURL.scheme ?? "https"
		components.host = sitePlannerURL.host ?? "site.meshtastic.org"
		components.path = "/"
		components.fragment = "cfg=\(sitePlannerConfigHash(for: payload))"
		return components.url ?? URL(string: "https://site.meshtastic.org")!
	}

	static func sitePlannerConfigHash(for payload: SitePlannerCoverageRequest) -> String {
		let transmitterPowerWatts = pow(10.0, (payload.txPower - 30.0) / 10.0)
		let params: [String: Any] = [
			"transmitter": [
				"name": "Meshtastic Node",
				"tx_lat": payload.lat,
				"tx_lon": payload.lon,
				"tx_power": transmitterPowerWatts,
				"tx_freq": payload.frequencyMHz,
				"tx_height": payload.txHeight,
				"tx_gain": payload.txGain
			],
			"receiver": [
				"rx_sensitivity": payload.signalThreshold,
				"rx_height": payload.rxHeight,
				"rx_gain": payload.rxGain,
				"rx_loss": payload.systemLoss
			],
			"environment": [
				"radio_climate": payload.radioClimate,
				"polarization": payload.polarization,
				"clutter_height": payload.clutterHeight,
				"ground_dielectric": payload.groundDielectric,
				"ground_conductivity": payload.groundConductivity,
				"atmosphere_bending": payload.atmosphereBending
			],
			"simulation": [
				"situation_fraction": payload.situationFraction,
				"time_fraction": payload.timeFraction,
				"simulation_extent": payload.radius / 1000.0,
				"high_resolution": payload.highResolution
			],
			"display": [
				"color_scale": payload.colormap,
				"min_dbm": payload.minDbm,
				"max_dbm": payload.maxDbm,
	...
	private static let messageHandlerName = "sitePlannerCoverage"

	private static let exportCaptureScript = """
	(() => {
		if (window.__meshtasticCoverageBridgeInstalled) return;
		window.__meshtasticCoverageBridgeInstalled = true;
		window.__meshtasticCoverageGeoJSONSent = false;
		...
		HTMLAnchorElement.prototype.click = function() {
			const download = (this.getAttribute("download") || "").toLowerCase();
			const blob = blobs.get(this.href);
			if (blob && (download.endsWith(".geojson") || blob.type === "application/geo+json")) {
				blob.text().then((text) => {
					window.__meshtasticCoverageGeoJSONSent = true;
					post({ type: "geojson", text });
				}).catch((error) => {
	...
	private static let automationScript = """
	(() => {
		if (window.__meshtasticCoverageStarted) return;
		window.__meshtasticCoverageStarted = true;
		...
		const run = async () => {
			try {
				const runButton = await waitFor(
					...
					60000,
					"Site Planner controls"
				);
				const contourButton = Array.from(document.querySelectorAll("button"))
					.find((button) => text(button) === "Contours");
				contourButton?.click();
				runButton.click();

				await waitFor(() => {
					const alert = Array.from(document.querySelectorAll("[role='alert']"))
						.find((element) => text(element).length > 0);
					if (alert) throw new Error(text(alert));
					const button = document.getElementById("runSimulation");
					const siteButton = document.querySelector("button[title^='Go to']");
					return button && !button.disabled && siteButton;
				}, 300000, "coverage result");

				const siteButton = document.querySelector("button[title^='Go to']");
				siteButton.click();
				const exportButton = await waitFor(
					() => document.querySelector(".mt-popup-export button[data-fmt='geojson']"),
					15000,
					"GeoJSON export"
				);
				exportButton.click();
				await waitFor(
					() => window.__meshtasticCoverageGeoJSONSent,
					10000,
					"GeoJSON export data"
				);
			} catch (error) {
...
extension SitePlannerBrowserCoverageClient: WKNavigationDelegate {
	func webView(_ webView: WKWebView, didFinish navigation: WKNavigation!) {
		webView.evaluateJavaScript(Self.automationScript) { [weak self] _, error in
			if let error {
				Task { @MainActor in
					self?.finish(.failure(SitePlannerCoverageError.failed(error.localizedDescription)))
				}
			}
		}
	}
	...
	func webView(_ webView: WKWebView, didFail navigation: WKNavigation!, withError error: Error) {
		finish(.failure(SitePlannerCoverageError.failed(error.localizedDescription)))
	}
...
extension SitePlannerBrowserCoverageClient: WKScriptMessageHandler {
	func userContentController(_ userContentController: WKUserContentController, didReceive message: WKScriptMessage) {
		guard let body = message.body as? [String: Any],
			  let type = body["type"] as? String else {
			finish(.failure(SitePlannerCoverageError.invalidResponse))
			return
		}
		...
		case "geojson":
			guard let text = body["text"] as? String,
				...
				finish(.failure(SitePlannerCoverageError.missingFeatureCollection))
				return
			}
			do {
				let normalizedData = try SitePlannerCoverageClient.normalizedFeatureCollectionData(from: data)
				finish(.success(normalizedData))
			} catch {
				finish(.failure(error))
			}
		case "error":
			let message = body["message"] as? String
			finish(.failure(SitePlannerCoverageError.failed(message)))
		default:
			finish(.failure(SitePlannerCoverageError.unexpectedStatus(type)))
		}
	}
}
